...
function createProvider2(t: any, name: string): Promise<EthereumProvider> {
  return createProvider(t.config, name, t.artifacts);
}
extendEnvironment((hre) => {
  ...
  hre.getProvider = async function getProvider(
    ...
    if (!providers[name]) {
      providers[name] = await createProvider2(this, name);
    }
  ...
  hre.changeNetwork = async function changeNetwork(newNetwork: string) {
    ...
    console.log("newwork: ", this.network);
